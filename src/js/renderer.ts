...
scode.ready(() => {
    // Loads the settings from .scode
    scode.settings = new Settings();

    // Creates a new tabmaanger and loads the last files that were opened when scode has been closed the last time
    ...
    if (scode.settings.isFirstUse == true) {
        //scode.tabmanager.newTab(__dirname + '/../res/readme.md', false, false);
        scode.tabmanager.newTab(
            __dirname + '/../../LICENSES.html',
            false,
            false
        );
    }
